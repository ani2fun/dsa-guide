...
The **DSA** book for [Synapse](https://synapse.kakde.eu). This repository *is* the book: the
chapters sit at the root, and Synapse mounts it at `/synapse/dsa`.
...
- **Chapters are directories, lessons are `.md` files.** Ordering comes from the `NN-` prefix
  ...
  becomes `/synapse/dsa/first-steps/variables`. An `index.md` sorts first; anything unnumbered sorts last,
  alphabetically.
...
Assets live at `_media/dsa/<lesson-slug>/…` and are referenced app-absolutely:
`![alt](/media/dsa/<lesson-slug>/diagram.svg)`. Supported: svg, png, webp, jpg, gif, mp4, webm.

## Links

Intra-book links are app-absolute and are **not** rewritten: `/synapse/dsa/<chapter>/<lesson>`. Relative
